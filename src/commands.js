...
async function handleSmpStats(interaction) {
  ...
  await interaction.editReply({
    embeds: [
      new EmbedBuilder().setColor(0xF4A460).setTitle(`🍩 ${username} — DonutSMP Stats`)
        ...
        .setFooter({ text: "Obno's Slave • DonutSMP Stats" }).setTimestamp(),
    ],
  });
}
...
async function handleSmpLeaderboard(interaction) {
  ...
  await interaction.editReply({
    embeds: [
      new EmbedBuilder().setColor(0xF4A460).setTitle(`🍩 DonutSMP Leaderboard — ${label}`)
        .setDescription(lines.join('\n')).setFooter({ text: `Obno's Slave • Page ${page}` }).setTimestamp(),
    ],
  });
}
