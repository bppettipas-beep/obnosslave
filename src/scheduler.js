...
async function runRound(client, guildId) {
  ...
  if (announceChannel) {
    ...
    await announceChannel.send({
      embeds: [
        new EmbedBuilder().setColor(0xF4A460).setTitle('🍩 New Donut Round!')
          ...
          .addFields(roundFields).setFooter({ text: "Obno's Slave" }).setTimestamp(),
      ],
    }).catch(() => {});
  }
...
async function sendPairingDMs(client, pairing) {
  ...
  const embed = partnerId => new EmbedBuilder().setColor(0xF4A460).setTitle('🍩 You have a new Donut!')
    ...
    .setFooter({ text: "Obno's Slave • Reply /donut skip if you need to pass" }).setTimestamp();
